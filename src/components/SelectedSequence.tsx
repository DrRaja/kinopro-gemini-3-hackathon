  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 sequence-panel"
      >
        <div className="px-8 py-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-slate-400">{selectedFrames.length} frames - Drag to reorder</p>
            </div>
          </div>
...
function DraggableFrame({
  ...
  return (
    <motion.div
      ...
      layout
      className={`relative flex-shrink-0 group ${isDragging ? 'opacity-50' : ''} ${
        ...
        <div className="relative rounded-xl overflow-hidden border-2 border-slate-700 bg-slate-900">
          <div className="aspect-video relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3">
              <p className="text-xs font-bold text-white truncate">{frame.emotionalBeat ?? frame.scene}</p>
